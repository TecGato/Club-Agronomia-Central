...
export default function PostNoticias() {
  const arr = [
  ...
  const mainNews = arr[0];
  const otherNews = arr.slice(1);
    ...
    return (
      <div className="flex flex-wrap justify-center">
        <div className="sm:grid sm:grid-cols-2 sm:max-h-72 shadow-md overflow-hidden cursor-pointer hover:scale-105 transition ease-in-out mb-5 p-5">
          <img src={mainNews.image} alt={mainNews.title} loading="lazy" className="col-span-1 p-5 sm:order-2" />
          <div className="sm:col-span-1 sm:grid sm:grid-row-2 sm:order-1">
            <h3 className="text-3xl place-self-center text-center font-bold text-gray-700 p-5 sm:row-span-1">{mainNews.title}</h3>
            <p className="text-base text-gray-700 leading-6 pt-0 px-5 pb-5 mb-3 sm:row-span-1">{mainNews.post}</p>
          </div>
        </div>
        ...
        {otherNews.map((news) => {
          return (
            <div className="max-w-xs m-1 shadow-md overflow-hidden cursor-pointer hover:scale-105 transition ease-in-out p-5">
              <img src={news.image} alt={news.title} loading="lazy" className="" />
              <h3 className="text-lg text-center font-bold text-gray-700 px-5 pt-5 pb-3">{news.title}</h3>
              <p className="text-base text-gray-700 leading-6 px-5 pt-0">{news.post}</p>
            </div>
          )
        }
          )}
