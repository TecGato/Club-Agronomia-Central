...
export function Layout({ children, image, title }) {
  ...
  return (
    <>
      ...
      <Navbar />
      <main className={`w-full ${pathname === '/' ? 'p-0' : 'py-14'} min-h-screen duration-500 dark:!text-white dark:bg-[#171717]`}>
        {title && <HeaderTitle image={image} title={title} />}
        
        <div className={`w-full ${pathname === '/' ? '' : 'lg:max-w-5xl'}  mx-auto`}>{children}</div>
        <WhatsAppWidget
