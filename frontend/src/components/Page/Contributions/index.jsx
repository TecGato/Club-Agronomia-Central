...
export function Contributions({ visibleContributions }) {
  ...
  const { handleInput, handleClick, preferenceId, contribution } = useContributions()
    ...
    <div className="flex bg-gray-900/80 backdrop-blur-sm justify-center items-center overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
      <div className="bg-white rounded container mx-auto lg:max-w-[500px] w-full md:max-w-[450px] max-w-[343px]  pb-10 relative">
        ...
        <div className="flex justify-center items-center mt-2">
          <WarningAlert title="Algo salió mal" description="Necesitas ingresar una cantidad !!"/>
        </div>

        <div className="flex justify-center items-center mt-2 w-full lg:px-0 md:px-12 px-4">
          <button
            className={`${!preferenceId ? "text-base font-semibold flex justify-center items-center text-center text-xl leading-none text-gray-900 py-4 lg:max-w-[400px] lg:w-full md:w-full w-full bg-[#fbbf24] hover:bg-[#fb923c] transition duration-300 ease-in-out" : "hidden"}`}
            onClick={handleClick}
          >
