...
export function QuinchoCalendarContainer({ props }) {
  ...
  return (
    <Layout title={'Disponibilidad Quincho'} image={''}>
      <div>
        ...
        <div class="grid flex-grow w-full h-auto grid-cols-7 grid-rows-5 gap-px pt-px mt-1 bg-gray-200">
          {eventDates.map((dateEv) => {
            return dateEv.date !== '2023-05-25' &&
              dateEv.date !== '2023-05-29' &&
              dateEv.date !== '2023-05-01' ? (
              <QuinchoCalendarDay
                key={dateEv.date}
        ...
        {showModal ? <QuinchoCalendarForm setShowModal={setShowModal} /> : null}

        {/* Si el estado del card detail es true, renderiza card detail */}
        {showCard ? (
          <QuinchoCardDetail showCard={showCard} setShowCard={setShowCard} />
        ) : null}
      </div>
    </Layout>
  );
